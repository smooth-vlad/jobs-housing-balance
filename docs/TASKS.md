...
**Детали:**
- [x] Подписаться на события мыши (`OnMouseDown`, `OnMouseMove`, `OnMouseUp`) ✅
- [x] Сохранять смещение от позиции курсора до центра кнопки при начале перетаскивания ✅
- [x] Обновлять позицию кнопки во время перетаскивания ✅
- [x] Ограничить перемещение в пределах экрана (не выходить за границы) ✅
- [x] Обновлять позицию в реальном времени при движении мыши ✅

**Файлы:** `src/UI/IconButton.cs` (расширить метод)

**Примечание:** Реализованы обработчики мыши для перетаскивания кнопки. Кнопка отслеживает нажатие мыши, сохраняет смещение курсора и обновляет позицию при движении мыши. Добавлено ограничение перемещения в пределах границ экрана. Установлено `isInteractive = true` для получения событий мыши.

#### 9.3 Обработка кликов по кнопке ✅
**Цель:** Реагировать на клик по кнопке (не во время перетаскивания)

**Детали:**
- [x] Различать клик и перетаскивание ✅
- [x] Определять клик как `mouseDown -> mouseMove < threshold -> mouseUp` ✅
- [x] При клике вызвать метод открытия UI панели ✅
- [x] Игнорировать клик, если было перетаскивание ✅

**Файлы:** `src/UI/IconButton.cs`

**Технические детали:**
- Использован стандартный паттерн `eventClick` для CS моддинга
- Добавлен механизм подавления кликов после драга (`suppressNextClick`)
- Реализован anti-spam cooldown (150ms) для предотвращения множественных кликов
- Исправлен расчет `dragDistance` - теперь отслеживает общее расстояние от точки начала
- Порог драга: 6 пикселей (меньше = клик, больше = драг)
- Используется `p.Use()` для предотвращения всплытия событий
- Добавлен метод `OnButtonActivated()` с Debug.Log для тестирования

**Примечание:** Реализована полная обработка кликов с учетом перетаскивания. Кнопка корректно различает клики и драги, имеет защиту от спама и готова для интеграции с UI панелью в Task 9.4.
...
- [x] Кнопку можно перетаскивать по экрану ✅
- [x] При клике на кнопку срабатывает обработчик ✅
